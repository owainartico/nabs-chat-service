...
async function postToDiscord(email, draft) {
  ...
  const isCongrats = email.to === 'congratulations@nameabrightstar.com';
  const inboxLabel = isCongrats ? '🎉 NABS — congratulations@' : '📬 NABS — support@';
  const received = new Date(email.date).toLocaleString('en-AU', { timeZone: 'Australia/Sydney', dateStyle: 'medium', timeStyle: 'short' });
  const bodyPreview = email.body.slice(0, 400).replace(/\n{3,}/g, '\n\n').trim();

  const lines = [
    `**${inboxLabel}nameabrightstar.com**`,
    `**From:** ${email.from}`,
    `**Subject:** ${email.subject}`,
    `**Received:** ${received}`,
    ``,
    bodyPreview,
    bodyPreview.length < email.body.length ? `_…(truncated)_` : '',
  ];

  if (draft) {
    lines.push(``, `**💬 Suggested reply:**`, draft.slice(0, 500));
  }

  lines.push(``, `✏️ **Send reply:** <${replyUrl}>`);
