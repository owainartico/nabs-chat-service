...
const SERVICE_URL = process.env.SERVICE_URL || 'https://chat.nameabrightstar.com';
...
function isNoise(email) {
  // Don't filter Shopify contact forms as noise — they contain real customer messages
  if (isShopifyContactForm(email)) return false;
  return NOISE_PATTERNS.some(p => p.test(email.fromEmail) || p.test(email.subject));
}

// Detect Shopify contact form emails
function isShopifyContactForm(email) {
  return /mailer@shopify/i.test(email.fromEmail) && /customer message/i.test(email.subject);
}

// Extract real customer email/name from Shopify contact form body
function parseShopifyContactForm(email) {
  if (!isShopifyContactForm(email)) return email;

  const body = email.body || '';
  const emailMatch = body.match(/([\w.+-]+@[\w.-]+\.[a-z]{2,})/i);
  const nameMatch = body.match(/^([A-Z][a-z]+ [A-Z][a-z]+)/m)
    || body.match(/Name[:\s]+([^\n]+)/i);

  if (emailMatch) {
    console.log(`[shopify-form] Extracted real email: ${emailMatch[1]} from Shopify contact form`);
    return {
      ...email,
      fromEmail: emailMatch[1],
      from: nameMatch ? (nameMatch[1] + ' <' + emailMatch[1] + '>') : emailMatch[1],
      _shopifyForm: true,
      _originalFrom: email.fromEmail,
    };
  }

  console.log(`[shopify-form] Could not extract customer email from Shopify form — escalating`);
  return { ...email, _shopifyForm: true, _forceManual: true };
}
...
async function runPoll() {
  ...
  // Pre-process: parse Shopify contact forms + filter noise
  const processedEmails = allEmails
    .map(e => parseShopifyContactForm(e))
    .filter(e => {
      if (isNoise(e)) {
        console.log(`[email-monitor] Filtered noise: ${e.fromEmail} — ${e.subject}`);
        return false;
      }
      return true;
    });
  ...
  const triage = await triageEmails(processedEmails);
  const manualItems = [];
  const autoHandled = [];

  for (const t of triage) {
    const email = processedEmails[t.index];
    if (!email) continue;
    if (t.level === 'NOISE') continue;

    // Force manual if Shopify form couldn't extract email
    if (email._forceManual) {
      t.action = 'manual';
      t.summary = (t.summary || email.subject) + ' (Shopify form — no email extracted)';
    }

    // Repeat contact check: if we already replied to this email address, escalate
    if (!email._forceManual) {
      const isRepeat = hasReceivedReply(email.fromEmail);
      if (isRepeat) {
        console.log(`[email-monitor] Repeat contact detected: ${email.fromEmail}`);
        manualItems.push({ triage: { ...t, urgent: true, summary: t.summary + ' [REPEAT]' }, email });
        continue;
      }
    }
