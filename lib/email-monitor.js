...
const SMTP_CONFIG = {
  ...
  auth: {
    user: process.env.SMTP_USER || 'support@nameabrightstar.com',
...
LEVEL:
- RED: urgent â€” certificate broken, angry/frustrated customer, multiple follow-ups, long wait
- YELLOW: routine â€” simple question, wants cert link, name change request, asking where star is
...
- "manual": needs human review â€” refund request, dispute, complex/unclear situation
...
[{"index":0,"level":"RED"|"YELLOW"|"NOISE","action":"send_cert"|"name_change"|"manual","summary":"Sender name â€” one line description","extracted_name":"Exact New Name or null"}]
...
${emails.map((e, i) => `[${i}] From: ${e.from}\nEmail: ${e.fromEmail}\nSubject: ${e.subject}\nBody: ${e.body.slice(0, 400)}`).join('\n\n')}`;
  ...
  try {
    ...
    return emails.map((_, i) => ({ index: i, level: 'YELLOW', action: 'manual', summary: `${emails[i].from} â€” ${emails[i].subject}`, extracted_name: null }));
  }
}
...
async function sendReply({ to, from, subject, text, html }) {
  ...
  await transporter.sendMail({
    from: `"Name a Bright Star" <${from || 'support@nameabrightstar.com'}>`,
    to,
    subject,
    text,
    html,
...
async function autoHandleSendCert(email, triage) {
  try {
    ...
    if (!result.found || !result.registrations?.length) {
      console.log(`[auto] No registration for ${email.fromEmail} â€” escalating`);
      return false;
    }
...
async function autoHandleNewCode(email, triage) {
  try {
    ...
    const text = `Hi ${firstName},\n\nNo problem â€” we've generated a fresh registration code for you:\n\n${newCode}\n\nJust head to ${registerUrl}, click "Register Your Star", and enter this code to get started.\n\nWarmest wishes,\nThe Name a Bright Star Team`;
    const html = `<p>Hi ${firstName},</p>
<p>No problem â€” we've generated a fresh registration code for you:</p>
...
async function autoHandleNameChange(email, triage) {
  ...
  try {
    const result = await db.lookupByEmail(email.fromEmail);
    if (!result.found || !result.registrations?.length) {
      console.log(`[auto] No registration for ${email.fromEmail} â€” escalating name change`);
      return false;
    }
    ...
    const text = `Hi,\n\nDone â€” we've updated the certificate for ${starName} to show the name "${newName}".\n\nHere's your updated certificate link:\n${certUrl}\n\nWarmest wishes,\nThe Name a Bright Star Team`;
    const html = `<p>Done â€” we've updated the certificate for <strong>${starName}</strong> to show the name <strong>"${newName}"</strong>.</p>
...
async function postManualToDiscord(items) {
  ...
  const lines = ['ðŸ“¬ **NABS â€” needs your attention**', ''];
  for (const { triage, email } of items) {
    const replyUrl = await storePendingReply(email);
    const line = `${triage.summary}  âœï¸ [Reply](<${replyUrl}>)`;
    if (triage.level === 'RED') lines.push(`ðŸ”´ **${line}**`);
    else lines.push(`ðŸŸ¡ ${line}`);
  }
...
async function postAutoSummaryToDiscord(handled) {
  ...
  const lines = ['âœ… **NABS â€” auto-handled**', ''];
  for (const { summary, action } of handled) {
    const icon = action === 'name_change' ? 'âœï¸' : 'ðŸ“¨';
...
function startMonitor() {
  if (!process.env.IMAP_PASS) {
    console.log('[email-monitor] IMAP_PASS not set â€” skipping email monitoring');
    return;
  }
